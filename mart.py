...
def cat_cat():
    colors={'Promotion Blue':'Blue','Promotion Red':'Red','No Promo':'grey'}
    pltcatcat = px.sunburst(dataframe, path=[x_axis_val,y_axis_val],values='Marketing Spend',
                  color=y_axis_val, color_discrete_map=colors, title=f" Proportion of {x_axis_val} by {y_axis_val} over 6 months")
    st.plotly_chart(pltcatcat, use_container_width=True, width = 1000)
